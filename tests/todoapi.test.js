...
describe('#Todo apis', () => {
    describe('POST /todos', () => {
        it('should save the todo ', (done) => {
            ...
            chai.request(app).post('/todos').set('x-auth',users[0].tokens[0].token).send({text}).end((err,res)=>{
        ...
        it('should not save the todo', (done) => {
            ...
            chai.request(app).post('/todos').set('x-auth',users[0].tokens[0].token).send({}).end((err,res)=>{
    ...
    describe('GET /todos/:id', () => {
        it('should get a todo', (done) => {
            chai.request(app).get(`/todos/${todos[0]._id.valueOf()}`).set('x-auth',users[0].tokens[0].token).end((err, res) => {
                expect(res.statusCode).to.equal(200);
                expect(res.body).to.be.a('object');
        ...
        it('should not get a todo', (done) => {
            chai.request(app).get(`/todos/${todos[1]._id.valueOf()}`).set('x-auth',users[0].tokens[0].token).end((err, res) => {
        ...
        it('should give a 404 if todo not found',(done)=>{
            ...
            chai.request(app).get(`/todos/${newId}`).set('x-auth',users[0].tokens[0].token).end((err,res)=>{
                expect(res.statusCode).to.equal(404);
                done();
            });
        });
        ...
        it('should give a 400 if todo is invalid',(done)=>{
            chai.request(app).get(`/todos/ffregfr`).set('x-auth',users[0].tokens[0].token).end((err,res)=>{
                expect(res.statusCode).to.equal(400);
                done();
            });
        });
    ...
    describe('DELETE /todos/:id',()=>{
        ...
        it('should delete a todo',(done)=> {
            chai.request(app).delete(`/todos/${id}`).set('x-auth', users[1].tokens[0].token).end((err, res) => {
                expect(res.statusCode).to.equal(200);
                expect(res.body).to.be.a('object');
                expect(res.body).to.have.property('todo');
                expect(res.body.todo).to.have.property('text');
        ...
        it('should not delete a todo',(done)=> {
            chai.request(app).delete(`/todos/${id}`).set('x-auth', users[0].tokens[0].token).end((err, res) => {
                expect(res.statusCode).to.equal(404);
                done();
            });
        });
            ...
            it('should give a 404 if todo not found',(done)=>{
                let newId = new ObjectId().valueOf();
                chai.request(app).delete(`/todos/${newId}`).set('x-auth',users[0].tokens[0].token).end((err,res)=>{
                    expect(res.statusCode).to.equal(404);
                    done();
                });
            });
            it('should give a 404 if todo is invalid',(done)=>{
                chai.request(app).delete(`/todos/ffregfr`).set('x-auth',users[0].tokens[0].token).end((err,res)=>{
                    expect(res.statusCode).to.equal(404);
                    done();
                });
            });
    ...
    describe('PUT /todos/:id',()=>{
        it('should update the todo',(done)=>{
            ...
            chai.request(app).put(`/todos/${id}`).set('x-auth',users[0].tokens[0].token).send({text,completed: true}).end((err,res)=>{
        ...
        it('should clear completedAt when todo is not completed',(done)=>{
            let id = todos[1]._id.valueOf();
            chai.request(app).put(`/todos/${id}`).set('x-auth',users[1].tokens[0].token).send({completed:false}).end((err,res)=>{
                expect(res.status).to.equal(200);
        ...
        it('should login user and return valid auth token',(done)=>{
            chai.request(app).post('/users/login').send({email,password}).end((err,res)=>{
                ...
                User.findOne({_id:users[1]._id}).then((user)=>{
                    ...
                    expect(user.tokens[1]).to.include({access:'auth',token:res.headers['x-auth']});
                    done();
                }).catch((err)=>done(err));
